...
public class SlidingPaneLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        switch (heightMode) {
            case ExploreByTouchHelper.INVALID_ID /*-2147483648*/:
                ...
                break;
            case SmbConstants.GENERIC_WRITE /*1073741824*/:
                maxLayoutHeight = (heightSize - getPaddingTop()) - getPaddingBottom();
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (r0 == -2) {
                    ...
                    i2 = lp.width;
                    if (r0 == -1) {
                        childWidthSpec = MeasureSpec.makeMeasureSpec(widthSize - horizontalMargin, SmbConstants.GENERIC_WRITE);
                    } else {
                        childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width, SmbConstants.GENERIC_WRITE);
                    }
                }
                i2 = lp.height;
                if (r0 == -2) {
                    ...
                    i2 = lp.height;
                    if (r0 == -1) {
                        childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, SmbConstants.GENERIC_WRITE);
                    } else {
                        childHeightSpec = MeasureSpec.makeMeasureSpec(lp.height, SmbConstants.GENERIC_WRITE);
                    }
                }
                                ...
                                if (canSlide) {
                                    ...
                                    if (child != r0) {
                                        if (lp.width >= 0) {
                                            ...
                                            if (skippedFirstPass) {
                                                childHeightSpec = MeasureSpec.makeMeasureSpec(child.getMeasuredHeight(), SmbConstants.GENERIC_WRITE);
                                            } else {
                                                i2 = lp.height;
                                                if (r0 != -2) {
                                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, ExploreByTouchHelper.INVALID_ID);
                                            ...
                                            child.measure(MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, SmbConstants.GENERIC_WRITE), childHeightSpec);
                                        }
                                    }
                                }
                                if (lp.weight <= 0.0f) {
                                    ...
                                    if (canSlide) {
                                        child.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), SmbConstants.GENERIC_WRITE), childHeightSpec);
                                    } else {
                                        newWidth = widthSize - (lp.leftMargin + lp.rightMargin);
                                        childWidthSpec = MeasureSpec.makeMeasureSpec(newWidth, SmbConstants.GENERIC_WRITE);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        float x;
        float y;
        switch (ev.getAction() & MKeyEvent.KEYCODE_SLEEP) {
            case SID.SID_TYPE_USE_NONE /*0*/:
                x = ev.getX();
                y = ev.getY();
                this.mInitialMotionX = x;
