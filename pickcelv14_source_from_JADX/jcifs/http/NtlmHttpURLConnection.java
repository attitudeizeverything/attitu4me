...
public class NtlmHttpURLConnection extends HttpURLConnection {
    ...
    private void doHandshake() throws IOException {
        ...
        if (response == MKeyEvent.KEYCODE_HAIER_TASK || response == 407) {
            try {
    ...
    private NtlmMessage attemptNegotiation(int response) throws IOException {
        ...
        if (errorStream == null || errorStream.available() == 0) {
            ...
            password2 = Type3Message.getDefaultPassword();
            ...
            if (user != null) {
                ...
                try {
                    ...
                    auth = Authenticator.requestPasswordAuthentication(null, port, protocol, "", this.authMethod);
                    if (auth != null) {
                        return null;
                    }
        ...
        do {
        } while (errorStream.read(new byte[HttpTransport.DEFAULT_CHUNK_LENGTH], 0, HttpTransport.DEFAULT_CHUNK_LENGTH) != -1);
        if (response != 401) {
            authHeader = "Proxy-Authenticate";
            this.authProperty = "Proxy-Authorization";
        } else {
