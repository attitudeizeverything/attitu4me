...
abstract class NameServicePacket {
    ...
    static void writeInt2(int val, byte[] dst, int dstIndex) {
        ...
        dst[dstIndex] = (byte) ((val >> AUTHORITY_OFFSET) & MKeyEvent.KEYCODE_SLEEP);
    ...
    static int readInt2(byte[] src, int srcIndex) {
        return ((src[srcIndex] & MKeyEvent.KEYCODE_SLEEP) << AUTHORITY_OFFSET) + (src[srcIndex + IN] & MKeyEvent.KEYCODE_SLEEP);
    }

    static int readInt4(byte[] src, int srcIndex) {
        return ((((src[srcIndex] & MKeyEvent.KEYCODE_SLEEP) << 24) + ((src[srcIndex + IN] & MKeyEvent.KEYCODE_SLEEP) << 16)) + ((src[srcIndex + SRV_ERR] & MKeyEvent.KEYCODE_SLEEP) << AUTHORITY_OFFSET)) + (src[srcIndex + 3] & MKeyEvent.KEYCODE_SLEEP);
    }
    ...
    int writeHeaderWireFormat(byte[] dst, int dstIndex) {
        ...
        dst[dstIndex + SRV_ERR] = (byte) ((this.isRecurDesired ? IN : QUERY) + (((((this.opCode << 3) & 120) + (this.isResponse ? TransportMediator.FLAG_KEY_MEDIA_NEXT : QUERY)) + (this.isAuthAnswer ? QUESTION_OFFSET : QUERY)) + (this.isTruncated ? SRV_ERR : QUERY)));
    ...
    int readHeaderWireFormat(byte[] src, int srcIndex) {
        ...
        this.isResponse = (src[srcIndex + SRV_ERR] & TransportMediator.FLAG_KEY_MEDIA_NEXT) != 0;
        ...
        if ((src[(srcIndex + SRV_ERR) + IN] & TransportMediator.FLAG_KEY_MEDIA_NEXT) == 0) {
            z = false;
        } else {
            z = true;
        }
