...
public class Name {
    ...
    int readWireFormat(byte[] src, int srcIndex) {
        ...
        int length = 15;
        for (int i = 0; i < 15; i++) {
            tmp[i] = (byte) (((src[((i * 2) + 1) + srcIndex] & MKeyEvent.KEYCODE_SLEEP) - 65) << 4);
            tmp[i] = (byte) (tmp[i] | ((byte) (((src[((i * 2) + 2) + srcIndex] & MKeyEvent.KEYCODE_SLEEP) - 65) & 15)));
            if (tmp[i] != 32) {
                length = i + 1;
            }
        }
        ...
        this.hexCode = ((src[srcIndex + TYPE_OFFSET] & MKeyEvent.KEYCODE_SLEEP) - 65) << 4;
        this.hexCode |= ((src[(srcIndex + TYPE_OFFSET) + 1] & MKeyEvent.KEYCODE_SLEEP) - 65) & 15;
    ...
    int readScopeWireFormat(byte[] src, int srcIndex) {
        ...
        int n = src[srcIndex] & MKeyEvent.KEYCODE_SLEEP;
        if (n == 0) {
            this.scope = null;
            srcIndex = srcIndex2;
            return 1;
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    n = src[srcIndex2] & MKeyEvent.KEYCODE_SLEEP;
                    if (n == 0) {
                        break;
                    }
