...
public class Encdec {
    ...
    public static int enc_uint16be(short s, byte[] dst, int di) {
        ...
        dst[di] = (byte) ((s >> TIME_1970_MILLIS_64LE) & MKeyEvent.KEYCODE_SLEEP);
        dst[di2] = (byte) (s & MKeyEvent.KEYCODE_SLEEP);
        return TIME_1970_SEC_32LE;
    }

    public static int enc_uint32be(int i, byte[] dst, int di) {
        int i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) ((i >> 24) & MKeyEvent.KEYCODE_SLEEP);
        di = i2 + TIME_1970_SEC_32BE;
        dst[i2] = (byte) ((i >> 16) & MKeyEvent.KEYCODE_SLEEP);
        i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) ((i >> TIME_1970_MILLIS_64LE) & MKeyEvent.KEYCODE_SLEEP);
        dst[i2] = (byte) (i & MKeyEvent.KEYCODE_SLEEP);
        return TIME_1904_SEC_32LE;
    }

    public static int enc_uint16le(short s, byte[] dst, int di) {
        int di2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) (s & MKeyEvent.KEYCODE_SLEEP);
        dst[di2] = (byte) ((s >> TIME_1970_MILLIS_64LE) & MKeyEvent.KEYCODE_SLEEP);
        return TIME_1970_SEC_32LE;
    }

    public static int enc_uint32le(int i, byte[] dst, int di) {
        int i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) (i & MKeyEvent.KEYCODE_SLEEP);
        di = i2 + TIME_1970_SEC_32BE;
        dst[i2] = (byte) ((i >> TIME_1970_MILLIS_64LE) & MKeyEvent.KEYCODE_SLEEP);
        i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) ((i >> 16) & MKeyEvent.KEYCODE_SLEEP);
        dst[i2] = (byte) ((i >> 24) & MKeyEvent.KEYCODE_SLEEP);
        return TIME_1904_SEC_32LE;
    }

    public static short dec_uint16be(byte[] src, int si) {
        return (short) (((src[si] & MKeyEvent.KEYCODE_SLEEP) << TIME_1970_MILLIS_64LE) | (src[si + TIME_1970_SEC_32BE] & MKeyEvent.KEYCODE_SLEEP));
    }

    public static int dec_uint32be(byte[] src, int si) {
        return ((((src[si] & MKeyEvent.KEYCODE_SLEEP) << 24) | ((src[si + TIME_1970_SEC_32BE] & MKeyEvent.KEYCODE_SLEEP) << 16)) | ((src[si + TIME_1970_SEC_32LE] & MKeyEvent.KEYCODE_SLEEP) << TIME_1970_MILLIS_64LE)) | (src[si + TIME_1904_SEC_32BE] & MKeyEvent.KEYCODE_SLEEP);
    }

    public static short dec_uint16le(byte[] src, int si) {
        return (short) ((src[si] & MKeyEvent.KEYCODE_SLEEP) | ((src[si + TIME_1970_SEC_32BE] & MKeyEvent.KEYCODE_SLEEP) << TIME_1970_MILLIS_64LE));
    }

    public static int dec_uint32le(byte[] src, int si) {
        return (((src[si] & MKeyEvent.KEYCODE_SLEEP) | ((src[si + TIME_1970_SEC_32BE] & MKeyEvent.KEYCODE_SLEEP) << TIME_1970_MILLIS_64LE)) | ((src[si + TIME_1970_SEC_32LE] & MKeyEvent.KEYCODE_SLEEP) << 16)) | ((src[si + TIME_1904_SEC_32BE] & MKeyEvent.KEYCODE_SLEEP) << 24);
    }
    ...
    public static int enc_utf8(String str, byte[] dst, int di, int dlim) throws IOException {
        ...
        while (i2 < dlim && i < strlen) {
            int ch = str.charAt(i);
            if (ch < TIME_1970_SEC_32BE || ch > TransportMediator.KEYCODE_MEDIA_PAUSE) {
                if (ch <= 2047) {
                    ...
                    i2 = di + TIME_1970_SEC_32BE;
                    dst[di] = (byte) (((ch >> 0) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                    di = i2;
                    ...
                    i2 = di + TIME_1970_SEC_32BE;
                    dst[di] = (byte) (((ch >> TIME_1601_NANOS_64BE) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                    di = i2 + TIME_1970_SEC_32BE;
                    dst[i2] = (byte) (((ch >> 0) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                }
            } else {
                di = i2 + TIME_1970_SEC_32BE;
                dst[i2] = (byte) ch;
    ...
    public static String dec_utf8(byte[] src, int si, int slim) throws IOException {
        ...
        while (si2 < slim) {
            ...
            int ch = src[si2] & MKeyEvent.KEYCODE_SLEEP;
            if (ch == 0) {
                break;
            }
            if (ch < TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                ...
                ch = src[si] & MKeyEvent.KEYCODE_SLEEP;
                uni[ui] = (char) (uni[ui] | (ch & 63));
                if ((ch & 192) != TransportMediator.FLAG_KEY_MEDIA_NEXT || uni[ui] < '\u0080') {
                ...
                si2 = si + TIME_1970_SEC_32BE;
                ch = src[si] & MKeyEvent.KEYCODE_SLEEP;
                if ((ch & 192) != TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                    throw new IOException("Invalid UTF-8 sequence");
                }
                uni[ui] = (char) (uni[ui] | ((ch & 63) << TIME_1601_NANOS_64BE));
                si = si2 + TIME_1970_SEC_32BE;
                ch = src[si2] & MKeyEvent.KEYCODE_SLEEP;
                uni[ui] = (char) (uni[ui] | (ch & 63));
                if ((ch & 192) != TransportMediator.FLAG_KEY_MEDIA_NEXT || uni[ui] < '\u0800') {
                    throw new IOException("Invalid UTF-8 sequence");
                }
            } else {
