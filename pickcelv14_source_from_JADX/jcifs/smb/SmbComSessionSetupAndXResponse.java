...
class SmbComSessionSetupAndXResponse extends AndXServerMessageBlock {
    ...
    int readBytesWireFormat(byte[] buffer, int bufferIndex) {
        ...
        this.nativeLanMan = readString(buffer, bufferIndex, start + this.byteCount, MKeyEvent.KEYCODE_SLEEP, this.useUnicode);
        bufferIndex += stringWireLength(this.nativeLanMan, bufferIndex);
        if (!this.extendedSecurity) {
            this.primaryDomain = readString(buffer, bufferIndex, start + this.byteCount, MKeyEvent.KEYCODE_SLEEP, this.useUnicode);
