...
abstract class ServerMessageBlock extends Response implements Request, SmbConstants {
    ...
    static {
        ...
        header = new byte[]{(byte) -1, (byte) 83, (byte) 77, (byte) 66, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY, SMB_COM_CREATE_DIRECTORY};
    }
    ...
    static int readInt2(byte[] src, int srcIndex) {
        return (src[srcIndex] & MKeyEvent.KEYCODE_SLEEP) + ((src[srcIndex + 1] & MKeyEvent.KEYCODE_SLEEP) << 8);
    }

    static int readInt4(byte[] src, int srcIndex) {
        return (((src[srcIndex] & MKeyEvent.KEYCODE_SLEEP) + ((src[srcIndex + 1] & MKeyEvent.KEYCODE_SLEEP) << 8)) + ((src[srcIndex + 2] & MKeyEvent.KEYCODE_SLEEP) << 16)) + ((src[srcIndex + 3] & MKeyEvent.KEYCODE_SLEEP) << 24);
    }
        ...
        if (useUnicode) {
            try {
                ...
                if (LogStream.level > 0) {
                    ...
                    if (maxLen < TransportMediator.FLAG_KEY_MEDIA_NEXT) {
                        i = maxLen + 8;
                    }
    ...
    int encode(byte[] dst, int dstIndex) {
        ...
        dst[dstIndex] = (byte) ((this.wordCount / 2) & MKeyEvent.KEYCODE_SLEEP);
        dstIndex = dstIndex2 + this.wordCount;
        this.wordCount /= 2;
        this.byteCount = writeBytesWireFormat(dst, dstIndex + 2);
        dstIndex2 = dstIndex + 1;
        dst[dstIndex] = (byte) (this.byteCount & MKeyEvent.KEYCODE_SLEEP);
        dstIndex = dstIndex2 + 1;
        dst[dstIndex2] = (byte) ((this.byteCount >> 8) & MKeyEvent.KEYCODE_SLEEP);
    ...
    boolean isResponse() {
        return (this.flags & TransportMediator.FLAG_KEY_MEDIA_NEXT) == TransportMediator.FLAG_KEY_MEDIA_NEXT;
    }
    ...
    public String toString() {
        ...
        return new String("command=" + c + ",received=" + this.received + ",errorCode=" + str + ",flags=0x" + Hexdump.toHexString(this.flags & MKeyEvent.KEYCODE_SLEEP, 4) + ",flags2=0x" + Hexdump.toHexString(this.flags2, 4) + ",signSeq=" + this.signSeq + ",tid=" + this.tid + ",pid=" + this.pid + ",uid=" + this.uid + ",mid=" + this.mid + ",wordCount=" + this.wordCount + ",byteCount=" + this.byteCount);
    }
}
