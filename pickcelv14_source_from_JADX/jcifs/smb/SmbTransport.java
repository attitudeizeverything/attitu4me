...
public class SmbTransport extends Transport implements SmbConstants {
    ...
    SmbTransport(UniAddress address, int port, InetAddress localAddr, int localPort) {
        this.sbuf = new byte[SmbNamedPipe.PIPE_TYPE_TRANSACT];
        this.key = new SmbComBlankResponse();
        this.sessionExpiration = System.currentTimeMillis() + ((long) SO_TIMEOUT);
    ...
    void ssn139() throws IOException {
        ...
        do {
            ...
            switch (this.sbuf[0] & MKeyEvent.KEYCODE_SLEEP) {
                ...
                case SessionServicePacket.NEGATIVE_SESSION_RESPONSE /*131*/:
                    int errorCode = this.in.read() & MKeyEvent.KEYCODE_SLEEP;
                    switch (errorCode) {
    ...
    private void negotiate(int port, ServerMessageBlock resp) throws IOException {
        synchronized (this.sbuf) {
            ...
            Encdec.enc_uint32be(n & SupportMenu.USER_MASK, this.sbuf, 0);
            LogStream logStream = log;
            if (LogStream.level >= 4) {
            ...
            int size = Encdec.dec_uint16be(this.sbuf, 2) & SupportMenu.USER_MASK;
    ...
    protected void doConnect() throws IOException {
        ...
        if (resp.dialectIndex > 10) {
        ...
        } else if ((this.server.capabilities & ExploreByTouchHelper.INVALID_ID) == ExploreByTouchHelper.INVALID_ID || this.server.encryptionKeyLength == 8 || LM_COMPATIBILITY != 0) {
            ...
            if ((this.server.capabilities & ExploreByTouchHelper.INVALID_ID) == ExploreByTouchHelper.INVALID_ID) {
                this.capabilities |= ExploreByTouchHelper.INVALID_ID;
            }
    ...
    protected Request peekKey() throws IOException {
        while (Transport.readn(this.in, this.sbuf, 0, 4) >= 4) {
            if (this.sbuf[0] != -123) {
                ...
                while (true) {
                    if (this.sbuf[0] == null && this.sbuf[1] == null && this.sbuf[4] == (byte) -1 && this.sbuf[5] == 83 && this.sbuf[6] == 77 && this.sbuf[7] == 66) {
                        this.key.mid = Encdec.dec_uint16le(this.sbuf, 34) & SupportMenu.USER_MASK;
                        return this.key;
                    }
                    for (int i = 0; i < 35; i++) {
    ...
    protected void doSend(Request request) throws IOException {
        synchronized (BUF) {
            ...
            Encdec.enc_uint32be(SupportMenu.USER_MASK & n, BUF, 0);
            LogStream logStream = log;
            if (LogStream.level >= 4) {
                do {
                    log.println(smb);
    ...
    protected void doRecv(Response response) throws IOException {
        ...
        if ((this.capabilities & ExploreByTouchHelper.INVALID_ID) == ExploreByTouchHelper.INVALID_ID) {
            z = true;
        }
        resp.extendedSecurity = z;
        synchronized (BUF) {
            System.arraycopy(this.sbuf, 0, BUF, 0, 36);
            int size = Encdec.dec_uint16be(BUF, 2) & SupportMenu.USER_MASK;
            ...
            if (resp.command == 46 && (errorCode == 0 || errorCode == -2147483643)) {
                ...
                if (r.dataLength > 0) {
                    Transport.readn(this.in, r.f166b, r.off, r.dataLength);
                }
            } else {
