...
interface SmbConstants {
    ...
    static {
        int i;
        int i2 = FLAGS_COPY_TARGET_MODE_ASCII;
        LADDR = Config.getLocalHost();
        LPORT = Config.getInt("jcifs.smb.client.lport", SSN_LIMIT);
        MAX_MPX_COUNT = Config.getInt("jcifs.smb.client.maxMpxCount", DEFAULT_MAX_MPX_COUNT);
        SND_BUF_SIZE = Config.getInt("jcifs.smb.client.snd_buf_size", DEFAULT_SND_BUF_SIZE);
        ...
        FORCE_UNICODE = Config.getBoolean("jcifs.smb.client.useUnicode", USE_UNICODE);
        USE_NTSTATUS = Config.getBoolean("jcifs.smb.client.useNtStatus", true);
        SIGNPREF = Config.getBoolean("jcifs.smb.client.signingPreferred", USE_UNICODE);
        ...
        DEFAULT_FLAGS2 = (USE_UNICODE ? FLAGS2_UNICODE : SSN_LIMIT) | ((((USE_EXTSEC ? FLAGS2_EXTENDED_SECURITY_NEGOTIATION : SSN_LIMIT) | 3) | (SIGNPREF ? FLAGS_COPY_TARGET_MODE_ASCII : SSN_LIMIT)) | (USE_NTSTATUS ? FLAGS2_STATUS32 : SSN_LIMIT));
        if (USE_NTSMBS) {
            i = FLAGS_VERIFY_ALL_WRITES;
        } else {
            i = SSN_LIMIT;
        }
        i |= USE_NTSTATUS ? FLAGS_NOTIFY_OF_MODIFY_ACTION : SSN_LIMIT;
        if (!USE_UNICODE) {
            i2 = SSN_LIMIT;
        }
        DEFAULT_CAPABILITIES = (i | i2) | FLAGS2_RESOLVE_PATHS_IN_DFS;
        ...
        TCP_NODELAY = Config.getBoolean("jcifs.smb.client.tcpNoDelay", USE_UNICODE);
        ...
        CONN_TIMEOUT = Config.getInt("jcifs.smb.client.connTimeout", DEFAULT_SO_TIMEOUT);
        ...
        NULL_TRANSPORT = new SmbTransport(null, SSN_LIMIT, null, SSN_LIMIT);
    }
}
