...
class SmbComNegotiateResponse extends ServerMessageBlock {
    ...
    int readParameterWordsWireFormat(byte[] buffer, int bufferIndex) {
        ...
        this.server.securityMode = buffer[bufferIndex] & MKeyEvent.KEYCODE_SLEEP;
        ...
        this.server.encryptionKeyLength = buffer[bufferIndex] & MKeyEvent.KEYCODE_SLEEP;
    ...
    int readBytesWireFormat(byte[] buffer, int bufferIndex) {
        int start = bufferIndex;
        if ((this.server.capabilities & ExploreByTouchHelper.INVALID_ID) == 0) {
            ...
            if (this.byteCount > this.server.encryptionKeyLength) {
                ...
                try {
                    if ((this.flags2 & SmbConstants.FLAGS2_UNICODE) == SmbConstants.FLAGS2_UNICODE) {
                        do {
                            if (buffer[bufferIndex + len] == null && buffer[(bufferIndex + len) + 1] == null) {
                                this.server.oemDomainName = new String(buffer, bufferIndex, len, SmbConstants.UNI_ENCODING);
                            } else {
                                len += 2;
                            }
                        } while (len <= SmbNamedPipe.PIPE_TYPE_CALL);
                        throw new RuntimeException("zero termination not found");
                    }
                    while (buffer[bufferIndex + len] != null) {
                        len++;
                        if (len > SmbNamedPipe.PIPE_TYPE_CALL) {
                            throw new RuntimeException("zero termination not found");
                        }
                    }
