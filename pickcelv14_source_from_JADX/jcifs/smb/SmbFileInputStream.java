...
public class SmbFileInputStream extends InputStream {
    ...
    SmbFileInputStream(SmbFile file, int openFlags) throws SmbException, MalformedURLException, UnknownHostException {
        this.tmp = new byte[1];
        this.file = file;
        this.openFlags = openFlags & SupportMenu.USER_MASK;
        this.access = (openFlags >>> 16) & SupportMenu.USER_MASK;
        if (file.type != 16) {
            file.open(openFlags, this.access, TransportMediator.FLAG_KEY_MEDIA_NEXT, 0);
            this.openFlags &= -81;
        } else {
            file.connect0();
        }
    ...
    public int read() throws IOException {
        ...
        return this.tmp[0] & MKeyEvent.KEYCODE_SLEEP;
    }
    ...
    public int readDirect(byte[] b, int off, int len) throws IOException {
        ...
        this.file.open(this.openFlags, this.access, TransportMediator.FLAG_KEY_MEDIA_NEXT, 0);
        ...
        do {
            ...
            try {
                ...
                if (this.file.type == 16) {
                    request.remaining = HttpTransport.DEFAULT_CHUNK_LENGTH;
                    request.maxCount = HttpTransport.DEFAULT_CHUNK_LENGTH;
                    request.minCount = HttpTransport.DEFAULT_CHUNK_LENGTH;
                }
    ...
    public int available() throws IOException {
        ...
        if (this.file.type == 16) {
            try {
                ...
                this.file.open(32, pipe.pipeType & 16711680, TransportMediator.FLAG_KEY_MEDIA_NEXT, 0);
