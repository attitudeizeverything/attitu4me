...
class NetServerEnum2Response extends SmbComTransactionResponse {
    ...
    class ServerInfo1 implements FileEntry {
        ...
        public int getType() {
            return (this.type & ExploreByTouchHelper.INVALID_ID) != 0 ? 2 : 4;
        }
    ...
    int readDataWireFormat(byte[] buffer, int bufferIndex, int len) {
        ...
        for (int i = 0; i < this.numEntries; i++) {
            ...
            e.versionMajor = buffer[bufferIndex] & MKeyEvent.KEYCODE_SLEEP;
            bufferIndex = bufferIndex2 + 1;
            e.versionMinor = buffer[bufferIndex2] & MKeyEvent.KEYCODE_SLEEP;
            e.type = ServerMessageBlock.readInt4(buffer, bufferIndex);
            bufferIndex += 4;
            int off = ServerMessageBlock.readInt4(buffer, bufferIndex);
            bufferIndex += 4;
            e.commentOrMasterBrowser = readString(buffer, ((SupportMenu.USER_MASK & off) - this.converter) + start, 48, false);
