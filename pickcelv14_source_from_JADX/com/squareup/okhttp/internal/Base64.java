...
public final class Base64 {
    ...
    public static String encode(byte[] in) {
        ...
        for (int i2 = 0; i2 < end; i2 += 3) {
            i = index + 1;
            out[index] = MAP[(in[i2] & MKeyEvent.KEYCODE_SLEEP) >> 2];
            index = i + 1;
            out[i] = MAP[((in[i2] & 3) << 4) | ((in[i2 + 1] & MKeyEvent.KEYCODE_SLEEP) >> 4)];
            i = index + 1;
            out[index] = MAP[((in[i2 + 1] & 15) << 2) | ((in[i2 + 2] & MKeyEvent.KEYCODE_SLEEP) >> 6)];
            index = i + 1;
        ...
        switch (in.length % 3) {
            case SmbNamedPipe.PIPE_TYPE_RDONLY /*1*/:
                i = index + 1;
                out[index] = MAP[(in[end] & MKeyEvent.KEYCODE_SLEEP) >> 2];
                index = i + 1;
            ...
            case SmbNamedPipe.PIPE_TYPE_WRONLY /*2*/:
                ...
                out[index] = MAP[(in[end] & MKeyEvent.KEYCODE_SLEEP) >> 2];
                index = i + 1;
                out[i] = MAP[((in[end] & 3) << 4) | ((in[end + 1] & MKeyEvent.KEYCODE_SLEEP) >> 4)];
                i = index + 1;
