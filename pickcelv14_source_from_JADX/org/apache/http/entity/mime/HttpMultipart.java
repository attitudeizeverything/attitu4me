...
public class HttpMultipart {
    ...
    private void doWriteTo(HttpMultipartMode mode, OutputStream out, boolean writeContent) throws IOException {
        ...
        for (FormBodyPart part : this.parts) {
            ...
            switch (C03221.$SwitchMap$org$apache$http$entity$mime$HttpMultipartMode[mode.ordinal()]) {
                ...
                case SmbNamedPipe.PIPE_TYPE_WRONLY /*2*/:
                    writeField(part.getHeader().getField(MIME.CONTENT_DISPOSITION), this.charset, out);
                    if (part.getBody().getFilename() != null) {
                        writeField(part.getHeader().getField(MIME.CONTENT_TYPE), this.charset, out);
                        break;
                    }
                    break;
            }
