...
public class LinearLayoutSoftKeyboardDetect extends LinearLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        LOG.m138v(TAG, "We are in our onMeasure method");
        int height = MeasureSpec.getSize(heightMeasureSpec);
        int width = MeasureSpec.getSize(widthMeasureSpec);
        LOG.m140v(TAG, "Old Height = %d", Integer.valueOf(this.oldHeight));
        LOG.m140v(TAG, "Height = %d", Integer.valueOf(height));
        LOG.m140v(TAG, "Old Width = %d", Integer.valueOf(this.oldWidth));
        LOG.m140v(TAG, "Width = %d", Integer.valueOf(width));
        if (this.oldHeight == 0 || this.oldHeight == height) {
            LOG.m129d(TAG, "Ignore this event");
        } else if (this.screenHeight == width) {
            int tmp_var = this.screenHeight;
            this.screenHeight = this.screenWidth;
            this.screenWidth = tmp_var;
            LOG.m138v(TAG, "Orientation Change");
