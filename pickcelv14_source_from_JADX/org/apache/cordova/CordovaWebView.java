...
public class CordovaWebView extends WebView {
    ...
    class C03002 implements Runnable {
        ...
        C03002(CordovaWebView cordovaWebView, String str) {
        ...
        public void run() {
            ...
            LOG.m132e(CordovaWebView.TAG, "CordovaWebView: TIMEOUT ERROR!");
    ...
    class C03013 implements Runnable {
        ...
        C03013(int i, CordovaWebView cordovaWebView, int i2, Runnable runnable) {
    ...
    class C03024 implements Runnable {
        ...
        C03024(Runnable runnable, CordovaWebView cordovaWebView, String str) {
    ...
    public void init(CordovaInterface cordova, CordovaWebViewClient webViewClient, CordovaChromeClient webChromeClient, List<PluginEntry> pluginEntries, Whitelist internalWhitelist, Whitelist externalWhitelist, CordovaPreferences preferences) {
        ...
        this.bridge = new CordovaBridge(this.pluginManager, new NativeToJsMessageQueue(this, cordova), this.cordova.getActivity().getPackageName());
    ...
    public void loadUrlIntoView(String url, boolean recreatePlugins) {
        ...
        this.cordova.getActivity().runOnUiThread(new C03024(new C03013(Integer.parseInt(getProperty("LoadUrlTimeoutValue", "20000")), this, currentLoadUrlTimeout, new C03002(this, url)), this, url));
    }

    void loadUrlNow(String url) {
        if (LOG.isLoggable(3) && !url.startsWith("javascript:")) {
            LOG.m129d(TAG, ">>> loadUrlNow()");
        }
        if (url.startsWith("file://") || url.startsWith("javascript:") || url.startsWith("about:") || this.internalWhitelist.isUrlWhiteListed(url)) {
            super.loadUrl(url);
        }
    }
    ...
    public void loadUrlIntoView(String url, int time) {
        if (!(url.startsWith("javascript:") || canGoBack())) {
            LOG.m131d(TAG, "loadUrlIntoView(%s, %d)", url, Integer.valueOf(time));
        }
        loadUrlIntoView(url);
    }
    ...
    public void showWebPage(String url, boolean openExternal, boolean clearHistory, HashMap<String, Object> hashMap) {
        LOG.m131d(TAG, "showWebPage(%s, %b, %b, HashMap", url, Boolean.valueOf(openExternal), Boolean.valueOf(clearHistory));
        if (clearHistory) {
            clearHistory();
        }
        if (!openExternal) {
            ...
            LOG.m141w(TAG, "showWebPage: Cannot load URL into webview since it is not in white list.  Loading into browser instead. (URL=" + url + ")");
        }
        try {
            ...
            LOG.m133e(TAG, "Error loading url " + url, e);
        }
    }
    ...
    private void sendJavascriptEvent(String event) {
        ...
        if (this.appPlugin == null) {
            LOG.m141w(TAG, "Unable to fire event without existing plugin");
        } else {
    ...
    public void handlePause(boolean keepRunning) {
        LOG.m129d(TAG, "Handle the pause");
    ...
    public void printBackForwardList() {
        ...
        for (int i = 0; i < currentSize; i++) {
            LOG.m129d(TAG, "The URL at index: " + Integer.toString(i) + " is " + currentList.getItemAtIndex(i).getUrl());
        }
    }
    ...
    public boolean startOfHistory() {
        ...
        LOG.m129d(TAG, "The current URL is: " + currentUrl);
        LOG.m129d(TAG, "The URL at item 0 is: " + url);
