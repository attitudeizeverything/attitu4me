        ...
        public boolean matches(Uri uri) {
            try {
                ...
                if (this.port == null || this.port.equals(Integer.valueOf(uri.getPort()))) {
                    return this.path == null || this.path.matcher(uri.getPath()).matches();
                } else {
                    return false;
                }
            } catch (Exception e) {
                LOG.m129d(Whitelist.TAG, e.toString());
                return false;
            }
        }
    }
        ...
        if (this.whiteList != null) {
            try {
                if (origin.compareTo("*") == 0) {
                    LOG.m129d(TAG, "Unlimited access to network resources");
                    this.whiteList = null;
                    return;
                }
                ...
                LOG.m131d(TAG, "Failed to add origin %s", origin);
            }
        }
    }
