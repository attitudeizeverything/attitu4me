    ...
    private String fullPathForLocalURL(Uri URL) {
        if (FILESYSTEM_PROTOCOL.equals(URL.getScheme()) && "localhost".equals(URL.getHost())) {
            ...
            if (URL.getQuery() != null) {
                path = new StringBuilder(String.valueOf(path)).append("?").append(URL.getQuery()).toString();
            }
            return path.substring(path.indexOf(47, 1));
        } else if ("content".equals(URL.getScheme())) {
            return Uri.encode(new StringBuilder(String.valueOf('/')).append(URL.getHost()).append(URL.getPath()).toString(), "/");
        } else {
            return null;
        }
    }
